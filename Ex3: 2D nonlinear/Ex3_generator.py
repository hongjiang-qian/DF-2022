...
F0=np.array([[0.1, 0.3], [0, 0.3]]) 
G0=np.array([[1,-0.3],[0,1]]) # \sg
H0=np.array([[1,1],[0,1]]) # G
...
def f(x,a):
    temp=(F0@x)*a
    return np.array([np.cos(temp[0]), np.sin(temp[1])]).reshape(2,1)
def s(x,a):
    return G0
# observation
def g(x,a):
...
#     T=0
#     T=T+np.random.exponential(0.3,1) # the first jump time
#     for i in range(1,N+1):
#         if(i*eta>=T):
#             alpha[i]=3-alpha[i-1]
#             # only when change status of mc,we update the next jump time.
#             sTime=np.random.exponential(0.3,1)
#             T=T+sTime
#         else:
#             alpha[i]=alpha[i-1]
#     return alpha
...
if __name__=='__main__':
    ...
    zip_file_path=path.join(dir_path, '../datas/ex3Train.pklz')
